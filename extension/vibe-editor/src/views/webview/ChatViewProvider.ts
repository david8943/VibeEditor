...
export class ChatViewProvider implements vscode.WebviewViewProvider {
  ...
  private currentPage: PageType = PageType.CHAT
    ...
    webview.onDidReceiveMessage(async (message: Message) => {
      ...
      switch (message.type) {
        ...
        case MessageType.WEBVIEW_READY:
          await this.navigate(PageType.CHAT)
          break
        ...
        case MessageType.USER_MESSAGE:
          await this.userMessage(message.payload)
          break
        case MessageType.AI_MESSAGE:
          await this.aiMessage(message.payload)
          break
        case MessageType.SHOW_SNAPSHOT:
          await this.showSnapshot(message.payload)
          break
      }
    }, this.disposables)
  }
  private async showSnapshot(snapshot: Snapshot) {
    await this.snapshotService.viewUpdatedSnapshot(snapshot)
  }
  private async userMessage(message: string) {
    // 1. 사용자 메시지를 히스토리에 추가
    this.chatHistory.push({ role: 'user', content: message })
    const userAIProviderId = Configuration.get(
      ConfigType.defaultUserAIProviderId,
    )
    // 2. generateChat 호출

    const aiResponse = await generateChat({
      userAIProviderId: userAIProviderId,
      message: message,
    })
    ...
    if (aiResponse.success) {
      this.chatHistory.push({
        role: 'assistant',
        content: aiResponse.data.aiResponse,
      })
      // 4. 전체 히스토리를 webview로 전송
      this.postMessageToWebview({
        type: MessageType.AI_MESSAGE,
        payload: {
          text: aiResponse.data.aiResponse,
  ...
  private async aiMessage(message: string) {
...
export function setChatViewProvider(provider: ChatViewProvider) {
  chatViewProviderInstance = provider
}
